...
- [ ] 「表から貼り付け」ボタン: Excel / Google スプレッドシート / CSV からコピーした
      クリップボード内容（タブ区切り／カンマ区切り）をパースしてフィールド一括登録
  - 2列（フィールド名, 型）＝選択中の型へ追加、3列（型名, フィールド名, 型）＝複数型を一括定義
  - ヘッダ行の自動判定、型名の妥当性チェック付きプレビューダイアログ → 確認して反映
- [ ] 逆方向（型定義を TSV でクリップボードへコピー）も対応
...
#### 5.4 スケジューリング設計（大規模・要件定義から）

- [ ] 要件定義: 対象範囲の確定（周期・位相（オフセット）設計 / レート整合チェック /
      Executor・コールバックグループ / プロセス配置（composition） / RT 優先度）
- [ ] モデル・GUI・生成物への反映設計 → 実装
