...
- [x] 各 OS 向けインストーラ（Tauri bundle: `.deb` / `.dmg` / `.msi`）
  - Linux はローカル検証済（2026-07-06: `.deb`/`.rpm`/`.AppImage` 生成、AppImage 起動確認、.deb メタデータ確認）
  - macOS / Windows は release.yml が各 OS ランナーでビルド（GitHub push + タグ後に初回確認）
- [ ] アンインストーラ動作確認 ← 実機での apt/dnf remove・MSI アンインストールは手動確認待ち（手順は doc/release.md）
- [x] バージョニング・リリースワークフロー（`npm run version:bump` + `v*` タグ → GitHub Releases ドラフト。稼働確認は push 後）
- [x] ユーザーマニュアル（`doc/user_manual.md`）

**Phase 4 準備完了（2026-07-06）** — 残りは GitHub push 後のリリースワークフロー稼働確認とアンインストール実機確認のみ。

---

### Phase 5: 機能拡張（ユーザー要望 2026-07-06）

Phase 4 までの MVP+多言語+抽象化を土台に、実運用で効く機能を追加する。

#### 5.1 msg 定義の表計算支援（小〜中規模）

大規模な msg 定義を型エディタで1件ずつ入力するのは非現実的なため、表計算ソフトからの一括投入を支援する。

- [x] 「表から貼り付け」ボタン: Excel / Google スプレッドシート / CSV からコピーした
      クリップボード内容（タブ区切り／カンマ区切り）をパースしてフィールド一括登録
  - 2列（フィールド名, 型）＝指定した単一型、3列（型名, フィールド名, 型）＝複数型を一括定義
  - ヘッダ行の自動判定、型名の妥当性チェック付きプレビューダイアログ → 確認して反映
  - 同名の既存型はフィールド置換（プレビューに明示）
- [x] 逆方向（型定義を TSV でクリップボードへコピー）も対応
- [x] vitest 導入（パーサ単体テスト8件、CI 組込み）

**5.1 完了（2026-07-06、GUI 手動確認待ち）**

- （発展）.csv / .xlsx ファイルの直接読込（xlsx は SheetJS 等。クリップボード対応で実用上は概ね充足）

#### 5.2 launch 作成の拡充（中規模）
...
- [x] 段階1: **実行時イントロスペクション（rqt_graph 方式）**（2026-07-07 完了）
  - `tools/introspect.py`（rclpy graph API で全ノードの Pub/Sub トピック・型・
    パラメータをサービス経由で JSON ダンプ）
  - `import_graph` コマンド + メニュー「インポート」: ノード/ポート/エッジ/namespace/
    パラメータを復元し、データフロー順の階層レイアウトで配置
  - **E2E 検証済**: Docker 内で実行中の3言語デモ（Rust/Python/C++）を解析 →
...
対象範囲（2026-07-06 ユーザー確認済み）:

- 周期・位相（オフセット）設計とレート整合チェック
- **実行順序の設計と「次の周期までに処理が終わるか」のデッドライン検証**
  （ノードの実行時間見積り（WCET）を持たせ、チェーンの応答時間を解析）
- Executor / コールバックグループ設計
...
- [x] 5.4c: コード生成反映（オフセット / プロセス runner / launch prefix）。Docker で
      プロセス統合（2ノード1プロセス）と疎通を検証（2026-07-07）

**5.4 完了（2026-07-07）** — 実測 WCET 取り込み・厳密応答時間解析・C++ プロセス統合は将来課題（scheduling_design.md §4）
...
5. ~~Phase 4（配布・運用）の準備~~ ✅ 準備完了（2026-07-06。残りは GitHub push 後の稼働確認）
6. GitHub への push + CI/リリースワークフロー稼働
7. Phase 5（機能拡張）: 5.1 表計算支援 → 5.2 launch 拡充 → 5.3 逆引き → 5.4 スケジューリング（優先順位は要確認）
...
- 2026-07-06: Phase 4 準備完了（Linux バンドル検証・release.yml・version:bump・release.md・user_manual.md。残りは push 後の稼働確認）
